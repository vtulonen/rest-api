...
import Url from 'url';
import {
  koaBody, artistPath, artistsPath, artistArtworksPath,
} from './constants';
...
artists.get(artistArtworksPath, checkAccept, async (ctx) => {
  const { id } = ctx.params;
  console.log('.get id contains:', id);
...
artists.put(artistPath, checkAccept, checkContent, koaBody, async (ctx) => {
  ...
  try {
    ...
    if (status.affectedRows === 0) { // If the resource does not already exist, create it
      await conn.execute(`
          INSERT INTO artists (id, firstName, lastName, nationality)
